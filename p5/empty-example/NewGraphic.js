...
let diagramaPoligono = [];

// Put any asynchronous data loading in preload to complete before "setup" is run
function preload()
{
  //carga el archivo y tiene en cuenta el titulo de las columnas
  table = loadTable("assets/datosInternetCompleto2.csv", "header");
}
...
function setup()
{
  ...
  for (let i = 0; i < rowCount; i++)
  {
   ...
   diagramaCirculos.push(new circulo (x,yComcel));
    ...
    diagramaPoligono.push(new poligono(x,yComcel));
...
class circulo
{
  ...
  {
    fill(255,25,219,170);
     polygon(this.posX,this.posY,5,6);
  }
}
...
class poligono 
{
  ...
  dibujar()
  {
    fill(0,255,0);
    push();    
    polygon(this.posX,this.posY,10,5);
    pop();
  }
}
